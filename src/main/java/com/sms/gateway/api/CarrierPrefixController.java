...
public class CarrierPrefixController {
    ...
    @GetMapping("/search")
    public Page<CarrierPrefix> search(
            @RequestParam(name = "prefix") String prefix,
            @RequestParam(name = "carrier", required = false) Carrier carrier,
            @RequestParam(name = "activeOnly", defaultValue = "true") boolean activeOnly,
            @RequestParam(name = "page", defaultValue = "0") int page,
            @RequestParam(name = "size", defaultValue = "50") int size
    ) {
        ...
        Pageable pageable = PageRequest.of(Math.max(page, 0), safeSize,
                Sort.by(Sort.Order.asc("carrier"), Sort.Order.asc("prefix")));
        return service.searchByPrefix(prefix, carrier, activeOnly, pageable);
    }
